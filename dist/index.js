...
var GENERIC_SKILL_TOKENS = new Set([
  "add",
  "all",
  "and",
  "api",
...
function defaultAgentDirs(projectDirectory) {
  const home = homedir();
  return [
...
function defaultSkillDirs(projectDirectory) {
  const home = homedir();
  return [
...
function normalizeDirs(dirs, defaults) {
  const expanded = [...defaults, ...dirs ?? []].map((dir) => dir.replace(/^~(?=$|\/)/, homedir())).map((dir) => resolve(dir));
...
async function loadAgents(agentDirs) {
...
async function loadSkills(skillDirs) {
  ...
  await Promise.all(files.map(async (path) => {
    ...
    if (!skillsById.has(id))
      skillsById.set(id, { id, name, description, path });
  }));
...
function scoreText(searchable, request, tokens) {
  let score = 0;
...
function scoreAgent(agent, request, tokens) {
  const haystack = `${agent.id} ${agent.name} ${agent.description}`.toLowerCase();
  let score = scoreText(haystack, request, tokens);
  if (haystack.includes("orchestrator"))
    score += 3;
  if (haystack.includes("senior") || haystack.includes("architect"))
    score += 2;
  return score;
}
function scoreSkill(skill, request, tokens) {
  const haystack = `${skill.id} ${skill.name} ${skill.description}`.toLowerCase();
  const hints = SKILL_HINTS[skill.id] ?? [];
  let score = 0;
  for (const hint of hints) {
    if (request.includes(hint))
      score += 30;
  }
  if (skill.id === "context7" && /\b(next|react|vue|angular|svelte|express|fastify|supabase|firebase|stripe|openai|sdk|package|library|framework)\b/.test(request))
    score += 30;
  if (skill.id === "task-management" && /\b(feature|project|multi-step|subtasks?|milestone|task list|todo)\b/.test(request))
    score += 24;
  ...
  if (hints.length > 0 && score < 30)
    return 0;
  return score;
}
function chooseThree(agents, requestText) {
  const request = requestText.toLowerCase();
  const tokens = tokenize(requestText);
  if (agents.length < 3)
    return;
  const ranked = agents.map((agent) => ({ agent, score: scoreAgent(agent, request, tokens) })).sort((a, b) => b.score - a.score || a.agent.name.localeCompare(b.agent.name));
  const primary = ranked[0]?.agent;
  const secondary = ranked.find((entry) => entry.agent.id !== primary?.id)?.agent;
  const verifierRanked = agents.filter((agent) => agent.id !== primary?.id && agent.id !== secondary?.id).map((agent) => {
...
function chooseSkills(skills, requestText, minSkillScore, maxSkills) {
  const request = requestText.toLowerCase();
  const tokens = tokenize(requestText);
  const ranked = skills.map((skill) => ({ skill, score: scoreSkill(skill, request, tokens) })).filter((entry) => entry.score >= minSkillScore).sort((a, b) => b.score - a.score || a.skill.name.localeCompare(b.skill.name));
  return ranked.slice(0, maxSkills).map((entry) => entry.skill);
}
function routingDirective(primary, secondary, tertiary) {
  return [
    "<tri-agent-routing>",
...
function skillDirective(skills, announceNoSkills) {
  if (skills.length === 0) {
    if (!announceNoSkills)
      return "";
    return [
...
function selectionSummary(primary, secondary, tertiary, skills) {
...
function createSelection(primary, secondary, tertiary, skills, originalText, announceNoSkills) {
  return {
    originalText,
...
function approvalPrompt(selection) {
  return [
    ...
    "[always] - approve this selection and auto-approve future router selections for the rest of this session.",
    "[autonomous approve] - tell the user which agents and skills were drafted, then continue the work without asking again for the rest of this session.",
...
function approvalAppliedPrefix(decision) {
  const scope = decision === "approve" ? "current request" : "rest of this session";
  return [
...
function denialPrompt(selection) {
  return [
...
function addRemovePrompt(selection, userEdit) {
  return [
...
function parseApprovalDecision(text) {
  ...
  if (normalized === "always")
    return "always";
  if (normalized === "autonomous approve" || normalized === "autonomous" || normalized === "auto approve")
    return "autonomous approve";
  if (normalized === "add/remove" || normalized === "add remove" || normalized === "add" || normalized === "remove")
    return "add/remove";
...
var TriAgentRouter = async ({ directory }, options) => {
  ...
  function stateFor(sessionID) {
    ...
    if (existing)
      return existing;
    const created = { mode: "ask", disabled: false };
    ...
    async "chat.message"(input, output) {
      ...
      if (state.pending) {
        ...
        if (decision === "always" || decision === "autonomous approve") {
          const pending = state.pending;
          state.mode = decision === "always" ? "always" : "autonomous";
          state.pending = undefined;
      ...
      if (!requireApproval || state.mode === "always" || state.mode === "autonomous") {
...
var src_default = TriAgentRouter;
export {
